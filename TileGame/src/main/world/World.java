...
import main.entity.creature.Slime;
...
//		}
	...
	public void saveWorld() {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				worldString += world[x][y].getId();
				worldString += " ";
			}
			worldString += "\n";
		}
		
		Utils.writeStringToFile(folderPath + localWorldPath, worldString);
		
		//Save entities
		String entitiesString = "";
		for (Entity e : entityManager.getEntities()) {
//			entitiesString += Arrays.asList(e.getClass());
			Utils.writeObjectToFile(folderPath + localEntitiesPath, e);
		}
