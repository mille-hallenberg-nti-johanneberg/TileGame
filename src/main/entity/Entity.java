...
public abstract class Entity implements Serializable{
	...
	public ArrayList<ItemEntity> intersectsWithItems(){
		ArrayList<ItemEntity> items = new ArrayList<>();
		
		for (Entity e : Handler.instance.getEntityManager().getEntities()) {
			if (e.getClass() != ItemEntity.class) continue;
	...
	public float getCenterX() {
		return x + width / 2 + bounds.x;
	}
	
	public float getCenterY() {
		return y + height / 2 + bounds.y;
	}
